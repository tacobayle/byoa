...
resource "null_resource" "ansible_avi" {
  ...
  provisioner "remote-exec" {
    inline = [
      "cd aviConfigure",
      "ansible-playbook -i ../hosts_avi local.yml --extra-vars '{\"vcenter\": ${jsonencode(var.vcenter)}, \"vmw\": ${jsonencode(var.vmw)}, \"vsphere_password\": ${jsonencode(var.vsphere_password)}, \"avi_vsphere_server\": ${jsonencode(var.avi_vsphere_server)}, \"vsphere_username\": ${jsonencode(var.vsphere_username)}, \"avi_username\": ${jsonencode("admin")}, \"avi_password\": ${jsonencode(random_string.avi_password.result)}, \"avi_version\": ${split("-", var.controller.version)[0]}, \"controllerPrivateIps\": ${jsonencode(vsphere_virtual_machine.controller.*.default_ip_address)}, \"controller\": ${jsonencode(var.controller)}}'"
    ]
  }
}
